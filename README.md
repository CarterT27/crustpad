...
Crustpad is a Rustpad-inspired collaborative code editor with browser-local code
execution. It keeps Rustpad's anonymous single-document room model and
operational transformation (OT) collaboration, then adds runnable language modes,
state persistence, reconnect/desync handling, and a Cloudflare Workers production
runtime.
...
Browser / Monaco
  <-> SyncClient OT state
  <-> WebSocket protocol
  <-> RoomObject or Bun server
  <-> Room in-memory state
  <-> Durable Object storage or SQLite snapshots

Browser / Run button
  -> language-specific Web Worker
  -> local stdout/stderr/error result
  -> OutputPanel
```
