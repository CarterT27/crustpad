...
import { isPersistedDocument, isUserOperations } from "./protocol";
...
const PERSISTED_ACCESS_TOUCH_INTERVAL = 5 * 60 * 1_000;
...
export class RoomObject extends DurableObject<Env> {
  ...
  private lastPersistedAccessAt = 0;
  private lastScheduledAccessAt = 0;
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    this.ready = ctx.blockConcurrencyWhile(async () => {
      ...
      this.room.beforeStateBroadcast = (state) => this.persistRoom(state);
      ...
      if (persisted.lastAccessedAt !== undefined) {
        ...
        this.lastPersistedAccessAt = persisted.lastAccessedAt;
        this.lastScheduledAccessAt = persisted.lastAccessedAt;
      }
    });
  }
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    try {
      ...
      const messageType = await room.handle(wrapper, message);
      this.saveAttachment(ws, wrapper);

      if (
        messageType &&
        messageType !== "edit" &&
        messageType !== "setLanguage"
      ) {
  ...
  async alarm(): Promise<void> {
    ...
    if (connectionCount === 0 && room.lastAccessedAt < cutoff) {
    ...
    if (connectionCount > 0) {
      ...
      await this.persistAccess(true);
      return;
    }

    await this.scheduleExpiry(room.lastAccessedAt);
  }
  ...
  private async persistRoom(state?: RoomPersistenceState): Promise<void> {
    ...
    if (
      nextState.lastAccessedAt - this.lastScheduledAccessAt >=
      PERSISTED_ACCESS_TOUCH_INTERVAL
    ) {
      await this.scheduleExpiry(nextState.lastAccessedAt);
      this.lastScheduledAccessAt = nextState.lastAccessedAt;
    }
    ...
    this.lastPersistedAccessAt = nextState.lastAccessedAt;
  }

  private async persistAccess(force = false): Promise<void> {
    const lastAccessedAt = this.getRoom().lastAccessedAt;
    if (
      !force &&
      lastAccessedAt - this.lastPersistedAccessAt < PERSISTED_ACCESS_TOUCH_INTERVAL
    ) {
      return;
    }

    await this.scheduleExpiry(lastAccessedAt);
    await this.ctx.storage.put("lastAccessedAt", lastAccessedAt);
    this.lastPersistedAccessAt = lastAccessedAt;
    this.lastScheduledAccessAt = lastAccessedAt;
  }
  ...
  private async scheduleExpiry(lastAccessedAt: number): Promise<void> {
    const expiryMs = this.expiryDays() * 24 * 60 * 60 * 1_000;
    await this.ctx.storage.setAlarm(
      Math.max(lastAccessedAt + expiryMs, Date.now() + 1_000),
    );
  }
